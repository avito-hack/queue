...
`avito-adapter` — mock-сервис готовых механизмов Avito, с которыми взаимодействуют сервисы очереди и тикетов. Пользователи, объявления и заказы читаются и изменяются через PostgreSQL repository.
...
Пользователи, объявления и заказы хранятся в PostgreSQL и сохраняются после перезапуска процесса.
